...
impl JackAstElem<VarDeclaration> {
    pub fn feed(&mut self, token: JackToken) {
        match (self.children_count(), &token) {
            (0, JackToken::Keyword(JackKeyword::Var)) => self.push_token(token),
...
impl IntoJackAstKind for VarDeclaration {
...
mod tests {
    ...
    async fn test_var_statement() {
        ...
        {
            let mut file = File::create("priv/test/gen/vars.xml").await.unwrap();

            for tokens in list_tokens {
                let mut new_elem: JackAstElem<VarDeclaration> = JackAstElem::default();
        ...
        let mut generated_file = File::open("priv/test/gen/vars.xml").await.unwrap();
        let mut cmp_file = File::open("priv/test/cmp/vars.xml").await.unwrap();
